  ...
  if (user) {
    ...
    if (isLocalDev) {
      ...
      await sendEmail(c.env, {
        ...
        to: email,
        subject: "Your Citadel sign-in link",
...
// cross-Worker into Worker 2's (CMS) /admin/dashboard.
//
...
app.get("/api/auth/verify", async (c) => {
  ...
  setCookie(c, "citadel_session", cookieValue, {
    httpOnly: true,
    secure: true,
    sameSite: "Lax",
    path: "/",
