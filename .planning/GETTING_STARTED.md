...
Add auth middleware to TanStack Start routes. **`createMiddleware` is a
root export of `@tanstack/react-start`** — `@tanstack/react-start/middleware`
doesn't exist in this version, same kind of stale-subpath issue as
`getCloudflareContext()`. Note: unlike the rest of this guide, this
specific middleware flow hasn't been run end-to-end yet this session — the
import paths below are confirmed correct against the installed package's
type declarations, but the full auth/redirect/cookie flow is Phase 3 scope
and still needs its own verification pass when you build it out:
